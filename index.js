...
const funcs = {
  ...
  fspull: async (args) => {
    ...
    if (res.length == 0) {
      console.log(chalk.yellow(`This folder doesn't exist in your project.`))
      return;
    }
    if (args[1] && !fs.existsSync(args[1])) {
      fs.mkdirSync(args[1]);
    }
    let lastfile = res[res.length - 1].path;

    await cli.getFiles(data, res, '', lastfile);
  },
  apipull: async (args) => {
    ...
    if (args[1]) {
      if (!function_list['Table'].find(func => func.id === args[1])) {
